...
<details>
  <summary>Details</summary>
Motivation: 探索Fokker-Planck方程的量子力学推导方法，并理解其与热力学熵的关系。

Method: 基于可逆的Schrödinger动力学，采用量子力学方法推导Fokker-Planck方程，并分析其路径积分表示。

Result: 推导出Fokker-Planck方程，并发现热力学熵对应于量子熵的粗粒化。

Conclusion: 由于推导中使用了时间无关哈密顿量的本征态表示，热力学熵必须对应于量子熵的粗粒化。
...
<details>
  <summary>Details</summary>
Motivation: 现有编译器将布局和路由独立处理，导致重排效率低下，影响量子计算的并行性和性能。

Method: 采用路由感知的布局方法，利用A*算法将兼容移动分组为并行重排步骤。

Result: 平均减少17%的重排时间，最佳情况下减少49%。

Conclusion: 该方法显著提升了量子计算的效率，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 探索量子态制备中两种错误同时存在时的临界行为，理解纠缠相变的普适性。

Method: 通过数值模拟（高斯费米子、张量网络和蒙特卡洛采样技术）研究双协议制备表面码和GHZ类猫态，分析纠缠相图和RG流。

Result: 发现微小偏离Clifford区域会导致非单调纠缠增长，渗流和Nishimori固定点均表现出多重分形特性。

Conclusion: 临界行为从渗流流向Nishimori普适性，多重分形特性为理解量子电路中的纠缠相变提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 研究非微扰体系中分子动力学的开放系统行为，为有机光伏和分子电子学设计提供指导。

Method: 利用离子阱系统，通过自旋-声子相互作用和库工程技术，模拟多模线性振动耦合模型中的电荷和激子转移。

Result: 发现简并模式在大能隙下增强转移速率，而非简并模式通过慢模路径降低能隙依赖性，扩大高效转移窗口。

Conclusion: 研究表明，仅增加一个振动模式即可重塑非微扰量子激发转移，为模拟多模振动过程提供了可扩展路径。
...
<details>
  <summary>Details</summary>
Motivation: 探索FTC在传感器中的应用潜力，特别是其量子Fisher信息的动态行为及其对传感器精度的极限影响。

Method: 通过理论分析FTC的QFI动力学，调节AC场参数诱导宏观猫态跃迁，并在不同初始状态下验证其性能。

Result: 实现了Heisenberg极限精度（QFI ∼ N²t²），并在FTC相变中捕捉到临界指数。

Conclusion: FTC作为AC传感器具有高精度和鲁棒性，尤其在非线性响应和相变区域表现突出。
...
<details>
  <summary>Details</summary>
Motivation: 探索彩票假设在量子多体系统中的应用，验证稀疏网络在量子计算任务中的有效性。

Method: 研究横向场伊辛模型（TFIM）和环面码（TC）两种量子哈密顿量，通过剪枝稀疏网络并独立训练，分析其性能。

Result: 稀疏网络在参数减少一个数量级后仍保持高精度，性能与子网络结构相关，且观察到普适标度行为和量子相变迹象。

Conclusion: 剪枝不仅提升性能，还增强了对哈密顿量物理的理解，验证了彩票假设在量子任务中的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 研究数据驱动解码（DDD）中训练数据的非典型特性，并探索如何通过理论和技术改进解码性能。

Method: 引入示例重要性理论，分析数据增强技术（如调整错误率）在解码中的效果，并通过实验验证。

Result: 实验表明，数据增强技术能显著提高解码准确性，同时揭示了该技术在类别不平衡和标签噪声之间的权衡。

Conclusion: 示例重要性和数据增强技术对改进实验相关的数据驱动解码器具有广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 量子数据学习中高效提取信息的协议设计仍具挑战性，现有工具有限，需扩展经典低阶方法以填补量子领域的空白。

Method: 扩展低阶方法至量子设置，利用状态设计与低阶硬度的联系，分析随机稀疏非局域哈密顿量Gibbs态及随机浅层量子电路态的学习。

Result: 首次证明随机稀疏非局域哈密顿量Gibbs态学习的信息-计算间隙，并展示自适应测量模型中的学习困难性。

Conclusion: 量子低阶方法为信息-计算间隙提供了新工具，揭示了量子学习问题的复杂性变化。
...
<details>
  <summary>Details</summary>
Motivation: 探索手性分子在光场作用下的几何响应，揭示对映体敏感观测量的起源。

Method: 利用微分几何工具，分析光驱动电子波函数在复极化空间中的参数依赖性。

Result: 发现对映体敏感观测量源于分子响应在这些空间中的几何特性。

Conclusion: 通过几何方法，阐明了手性分子光响应的对映体敏感性机制。
...
### [55] [Quantum cooling below absolute zero](https://arxiv.org/abs/2505.22791)
*Francesco Libbi,Lorenzo Monacelli,Boris Kozinsky*

Main category: quant-ph

TL;DR: 通过共振中红外脉冲抑制量子涨落，诱导SrTiO$_3$在非平衡状态下发生铁电相变。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用光脉冲控制和抑制量子涨落，改变量子系统的自由能景观，实现原本不可能的铁电相变。

Method: 使用共振中红外脉冲诱导非平衡态，结合基于第一性原理的机器学习势能面进行定量描述。

Result: 成功诱导SrTiO$_3$发生铁电相变，并预测了非平衡态的长寿命条件。

Conclusion: 首次展示了通过光脉冲控制和利用量子涨落，可以显著改变量子系统的行为，为新材料设计提供了新思路。
...
### [56] [Timelike Quantum Energy Teleportation in the Nambu-Jona-Lasinio Model](https://arxiv.org/abs/2505.22794)
*Fidele J. Twagirayezu*

Main category: quant-ph

TL;DR: 提出了一种基于1+1维Nambu-Jona-Lasinio（NJL）模型的量子能量隐形传态（QET）协议，利用局域Unruh-DeWitt探测器实现能量传递。


<details>
  <summary>Details</summary>
Motivation: 探索在相互作用量子场论中实现QET的可能性，并利用NJL模型的真空纠缠结构。

Method: 通过耦合Unruh-DeWitt探测器到费米场，利用经典信息传递实现能量提取，并通过量子电路模拟验证。

Result: 成功展示了能量传递的可行性，验证了NJL模型作为QET平台的潜力。

Conclusion: NJL模型为研究相互作用量子场论中的QET提供了新途径，并为实验实现奠定了基础。
...
### [57] [Provably Efficient Quantum Thermal State Preparation via Local Driving](https://arxiv.org/abs/2505.22816)
*Dominik Hahn,S. A. Parameswaran,Benedikt Placke*

Main category: quant-ph

TL;DR: 提出了一种近似制备量子热态的方案，仅需重复使用三种易于实现的组件，适用于当前或近期的量子设备。


<details>
  <summary>Details</summary>
Motivation: 量子热态制备在量子多体物理中具有重要意义，但现有方法需要大量量子计算资源，难以在当前设备上实现。

Method: 方案基于三种组件：(a) 哈密顿量的模拟；(b) 局部时间依赖的耦合；(c) 辅助量子比特的重置。

Result: 提供了与哈密顿量具体细节无关的严格性能保证。

Conclusion: 该方案为当前量子设备上的热态制备提供了实用且高效的解决方案。
...
### [58] [Enhanced Excited State Population and Coherence via Adiabatic Tunneling Ionization and Excitation](https://arxiv.org/abs/2505.22833)
*Chi-Hong Yuen*

Main category: quant-ph

TL;DR: 论文提出了一种新方法，通过隧穿电离和强场激发增强离子激发态和相干性，适用于化学和激光控制。


<details>
  <summary>Details</summary>
Motivation: 研究隧穿电离和强场激发之间的复杂相互作用，以增强离子激发态和相干性。

Method: 采用绝热方法，推导了两能级离子系统的半解析解，去除强场修饰。

Result: 激发态布居增强了一个数量级，相干性振幅在多周期脉冲下可提升四个数量级。

Conclusion: 该方法为利用强激光脉冲生成和控制电子激发态和相干性提供了新框架。
...
### [59] [Counterfactuals in Macroscopic Quantum Physics: Irreversibility, Measurement and Locality](https://arxiv.org/abs/2505.22834)
*Maria Violaris*

Main category: quant-ph

TL;DR: 探讨量子理论是否适用于所有尺度，通过构造理论和量子信息理论解决测量、不可逆性和局域性问题，并揭示量子热力学的新特征。


<details>
  <summary>Details</summary>
Motivation: 量子理论的普适性存在争议，尤其是测量、不可逆性和局域性可能对其构成挑战。

Method: 使用构造理论和量子信息理论，通过反事实陈述分析问题。

Result: 发现量子热力学的新特征，开发工具量化量子算符的非交换性，揭示纠缠和量子分支结构的信息存储方式。

Conclusion: 结果支持量子理论能够一致描述微观和宏观系统，包括观察者和热力学系统。
...
### [60] [Quantum Reservoir Computing for Corrosion Prediction in Aerospace: A Hybrid Approach for Enhanced Material Degradation Forecasting](https://arxiv.org/abs/2505.22837)
*Akshat Tandon,James Brown,Kenny Heitritter,Tarini Hardikar,Kanav Setia,Rene Boettcher,Klaus Schertler,Jasper Simon Krauser*

Main category: quant-ph

TL;DR: 论文提出了一种基于量子储层计算的洋葱量子储层计算（Onion QRC）方法，用于预测材料降解，通过同时演化多个小型量子储层来捕捉不同时间尺度，同时保持电路深度较小，从而提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 材料降解预测在多个行业中具有重要意义，而环境条件（如湿度和温度）是降解过程的重要驱动因素。量子机器学习虽有潜力，但存在训练困难和测量开销等问题，因此研究量子储层计算以解决时间序列预测任务是必要的。

Method: 采用洋葱量子储层计算（Onion QRC）方法，通过同时演化多个小型量子储层来捕捉不同时间尺度，并通过调整旋转角度或中电路测量次数来改变长期或短期记忆能力。此外，结合经典储层层进一步优化预测精度。

Result: Onion QRC在预测铝合金在不同环境条件下的降解时表现优于简单模型和单一经典储层。结合经典储层层后，预测精度进一步提升。

Conclusion: Onion QRC是一种有效的量子储层计算方法，能够捕捉多时间尺度的降解动态，同时保持电路深度较小，为材料降解预测提供了新的解决方案。
...
### [61] [Spectrally Resolved Higher Order Photon Statistics of Spontaneous Parametric Down Conversion](https://arxiv.org/abs/2505.22883)
*Jeffrey Carvalho,Chiran Wijesundara,Tim Thomay*

Main category: quant-ph

TL;DR: 研究SPDC光子统计与波长、泵浦功率和符合时间的关系，发现光子数随泵浦功率不对称增加，波长依赖性效率显著。


<details>
  <summary>Details</summary>
Motivation: 探索SPDC光子统计特性及其在量子计量、通信和计算中的应用。

Method: 使用四探测器Hanbury Brown和Twiss干涉仪研究信号光束的光子统计，以闲频光作为触发。

Result: 光子统计符合负二项分布，表明其为热光源特性。

Conclusion: SPDC的光子统计具有波长依赖性，为复杂光源检测提供了新视角。
...
### [62] [Markovian heat engine boosted by quantum coherence](https://arxiv.org/abs/2505.22902)
*Freddier Cuenca-Montenegro,Marcela Herrera,John H. Reina*

Main category: quant-ph

TL;DR: 量子相干性在噪声马尔可夫单量子比特热机中作为热力学资源的作用被评估，展示了量子奥托循环如何通过消耗噪声量子态的相干性超越经典效率极限。


<details>
  <summary>Details</summary>
Motivation: 研究量子相干性在热机中的资源作用，探索噪声环境下量子热机的性能。

Method: 使用量子奥托循环，计算Leggett-Garg时间相关性，模拟振幅和相位阻尼通道以及门级噪声。

Result: 振幅阻尼显著降低效率和可提取功，而相位阻尼影响较小。量子奥托循环可超越经典效率极限。

Conclusion: 量子相干性是提升热机效率的关键资源，噪声类型对性能有不同影响。
...
### [63] [Optimizing QUBO on a quantum computer by mimicking imaginary time evolution](https://arxiv.org/abs/2505.22924)
*Yahui Chai,Alice Di Tucci*

Main category: quant-ph

TL;DR: 提出了一种混合量子-经典算法，通过模拟虚时间演化电路（ITEMC）解决QUBO问题，显著减少测量开销，并通过迭代更新初始状态快速收敛到低能解。


<details>
  <summary>Details</summary>
Motivation: 解决QUBO问题时减少测量开销并提高收敛速度，同时验证算法在硬件上的可行性。

Method: 使用ITEMC电路优化参数，仅需单比特和双比特期望值，避免全能量评估；通过预排序优化量子门顺序，迭代更新初始状态。

Result: 经典模拟在150比特内达到0.99以上的近似比；硬件实验在IBM设备上验证了40、60和80比特的可行性。

Conclusion: 该算法在减少测量开销和提高收敛速度方面表现优异，且硬件实现结果与模拟退火一致。
...
### [64] [Superoscillations and Physical Applications](https://arxiv.org/abs/2505.22925)
*Andrew N. Jordan,John C. Howell,Nicholas Vamivakas,Ebrahim Karimi*

Main category: quant-ph

TL;DR: 本章选择性回顾了超振荡及其相关现象的物理实现与应用，介绍了超振荡的简单示例及其与量子力学弱值的关系，并探讨了电磁应用中的超振荡与超增长现象。


<details>
  <summary>Details</summary>
Motivation: 探讨超振荡及其相关现象的物理实现与应用，揭示其在电磁学和量子物理中的潜力。

Method: 通过回顾超振荡的简单示例及其与量子力学弱值的关系，分析电磁应用中的超振荡与超增长现象，并总结多种创建超振荡函数的方法。

Result: 总结了超振荡在光学超分辨率、噪声抑制等领域的应用，并提出了未来研究方向，如超雷达和量子物理中的广义超现象。

Conclusion: 超振荡及其相关现象在电磁学和量子物理中具有广泛的应用前景，未来研究可进一步探索其在其他波现象中的潜力。
...
### [65] [Variational Quantum Simulations of a Two-Dimensional Frustrated Transverse-Field Ising Model on a Trapped-Ion Quantum Computer](https://arxiv.org/abs/2505.22932)
*Ammar Kirmani,Elijah Pelofske,Andreas Bärtschi,Stephan Eidenbenz,Jian-Xin Zhu*

Main category: quant-ph

TL;DR: 该论文研究了在量子计算机上模拟强关联系统的基态性质，特别是二维受挫横向场伊辛模型（TFIM），并使用变分量子本征求解器（VQE）成功提取了磁性相信息。


<details>
  <summary>Details</summary>
Motivation: 由于经典计算技术在研究量子相变系统时的局限性，量子计算机成为研究强关联系统基态性质的理想平台。尽管NISQ设备的错误率较高，但仍可能通过模拟提取相信息。

Method: 使用VQE模拟二维受挫TFIM，比较其与精确对角化的结果，并在Quantinuum H1-1离子阱量子计算机上运行优化后的VQE电路。

Result: 实验显示，通过基态能量、能量导数和自旋关联函数，成功恢复了模型的磁性相，表明离子阱量子处理器在VQE方法限制内能可靠模拟强关联系统。

Conclusion: 研究表明，在VQE方法的限制下，离子阱量子处理器能够可靠地模拟强关联系统，为未来量子计算在复杂系统研究中的应用提供了支持。
...
### [66] [Page Curve of average subsystem entropy](https://arxiv.org/abs/2505.23011)
*Oscar C. O. Dahlsten*

Main category: quant-ph

TL;DR: 论文探讨了Page曲线的定义及其与黑洞信息悖论的联系。


<details>
  <summary>Details</summary>
Motivation: 研究Page曲线的行为及其在量子态平均下的子系统熵变化，以理解其上升和下降的原因，并探讨其与黑洞信息悖论的关系。

Method: 通过分析子系统熵的平均值随子系统大小的变化，描述了Page曲线的形成机制。

Result: Page曲线从0开始上升，在子系统占系统一半时达到峰值，随后下降回0，揭示了熵的对称性。

Conclusion: Page曲线的行为为理解量子态熵分布提供了重要视角，并与黑洞信息悖论有直接关联。
...
### [67] [Generating discrete time crystals through optimal control](https://arxiv.org/abs/2505.23021)
*Mrutyunjaya Sahoo,Rahul Ghosh,Bandita Das,Shishira Mahunta,Bodhaditya Santra,Victor Mukherjee*

Main category: quant-ph

TL;DR: 利用最优控制在多体量子系统中生成离散时间晶体（DTC），通过优化成本函数实现时间平移对称性自发破缺。


<details>
  <summary>Details</summary>
Motivation: 探索一种新方法，将DTC的形成视为优化问题，以找到非平凡的周期性控制脉冲和参数范围。

Method: 定义适当的成本函数并通过优化生成DTC，适用于有耗散和无耗散的多体量子系统。

Result: 成功生成了DTC，并讨论了实验实现的可能控制协议。

Conclusion: 该方法为DTC的生成提供了新思路，并展示了实验实现的潜力。
...
### [68] [Beyond mean-field dynamics of the Dicke model with non-Markovian dephasing](https://arxiv.org/abs/2505.23028)
*Anqi Mu,Nathan Ng,Andrew J. Millis,David R. Reichman*

Main category: quant-ph

TL;DR: 提出了一种基于密度矩阵的时间依赖投影算子形式，用于计算具有非马尔可夫局部浴和一全相互作用的系统的超平均场动力学。


<details>
  <summary>Details</summary>
Motivation: 研究光学腔中凝聚相系统的物理行为，特别是Dicke模型在非马尔可夫局部退相干浴中的动力学。

Method: 结合张量网络影响泛函，使用密度矩阵方法分析零温度下的Dicke模型动力学。

Result: 修正了平均场动力学中的虚假初始状态依赖性，并揭示了新的时间尺度。

Conclusion: 该方法适用于有限温度下具有一全相互作用的其他量子光学系统。
...
### [69] [On the generic increase of entropy in isolated systems](https://arxiv.org/abs/2505.23041)
*Zhiqiang Huang,Qing-yu Cai*

Main category: quant-ph

TL;DR: 该研究为孤立量子系统中的熵产生建立了一个普适机制，基于本征态热化假设（ETH），揭示了熵产生源于多体相互作用，并通过解析和数值模拟验证了对数熵标度。


<details>
  <summary>Details</summary>
Motivation: 解决量子多体系统中熵产生的普适性问题，统一观测熵与冯·诺依曼熵动力学，为量子技术中的熵控制提供理论基础。

Method: 采用基于解析的框架，结合非可积伊辛自旋链的数值模拟，分析熵产生的两种普适路径：相互作用诱导的能量展宽和时间粗粒化。

Result: 数值模拟验证了对数熵标度，自洽方程与数值结果高度一致，统一了熵的观测与动力学描述。

Conclusion: 该研究解决了关于熵标度的长期争议，为量子多体系统的热力学行为提供了预测工具，并为量子技术中的熵控制开辟了新途径。
...
### [70] [Efficient Quantum Approximate $k$NN Algorithm via Granular-Ball Computing](https://arxiv.org/abs/2505.23066)
*Shuyin Xia,Xiaojiang Tian,Suzhen Yuan,Jeremiah D. Deng*

Main category: quant-ph

TL;DR: 论文提出了一种基于粒球和量化的量子kNN算法（GB-QkNN），通过粒球减少数据处理量，利用HNSW方法加速搜索，并通过量化进一步降低时间复杂度。


<details>
  <summary>Details</summary>
Motivation: 传统和量子kNN算法在处理大量数据时存在速度瓶颈，需要更高效的解决方案。

Method: 结合粒球技术和HNSW方法，并通过量化优化距离计算等耗时步骤。

Result: 显著降低了kNN类算法的时间复杂度。

Conclusion: GB-QkNN通过粒球和量化技术有效提升了kNN算法的效率。
...
### [71] [Networked Quantum Services](https://arxiv.org/abs/2505.23074)
*Laszlo Gyongyosi,Sandor Imre*

Main category: quant-ph

TL;DR: 综述了网络量子服务的基本概念、最新成果、技术、平台和应用，并分析了实施基础和关键挑战。


<details>
  <summary>Details</summary>
Motivation: 量子计算和通信的快速发展推动了先进解决方案和服务的开发，网络量子服务通过量子计算机和量子网络为用户提供服务。

Method: 通过全面研究现有技术、平台和应用，分析实施基础。

Result: 总结了网络量子服务的现状，识别了关键挑战。

Conclusion: 网络量子服务在量子计算和通信领域具有重要潜力，但仍需解决关键挑战以实现广泛应用。
...
### [72] [Bridging the classical and quantum regimes in a dissipative Ising chain](https://arxiv.org/abs/2505.23096)
*Zhenming Zhang,Haowei Li,Wei Yi*

Main category: quant-ph

TL;DR: 研究了具有变化量子关联的耗散伊辛链的长时间动力学，发现其可通过插值相干哈密顿量的Lindblad主方程描述，经典极限下出现极限环，量子极限下趋于铁磁稳态，中间态则自发破缺空间平移对称性。


<details>
  <summary>Details</summary>
Motivation: 探讨耗散伊辛链在量子关联变化下的动力学行为，以理解经典与量子极限之间的过渡现象及其在开放量子系统中的表现。

Method: 采用系综平均形式，假设空间平移对称性，通过Lindblad主方程描述动力学，并引入插值相干哈密顿量。

Result: 经典极限下出现非线性运动方程和极限环；量子极限下系统趋于铁磁稳态；中间态自发破缺对称性，出现反铁磁稳态。

Conclusion: 模型为中间温度下的耗散里德堡气体提供了有效描述，揭示了量子开放系统中极限环的命运。
...
### [73] [Two identical 1D anyons with zero-range interactions: Exchange statistics, scattering theory, and anyon-anyon mapping](https://arxiv.org/abs/2505.23127)
*Raúl Hidalgo-Sacoto,Thomas Busch,D. Blume*

Main category: quant-ph

TL;DR: 该论文研究了1D玻色子和费米子任意子的散射框架，验证了玻色子-费米子任意子映射，并分析了动量分布和相关性。
...
Method: 开发了1D玻色子和费米子任意子的散射框架，通过零范围接触相互作用分析两粒子系统。

Result: 验证了玻色子-费米子任意子映射，动量分布和相关性存在差异，并讨论了高阶动量分布的贡献。

Conclusion: 研究为任意子统计提供了新的理论框架，并验证了其在实际系统中的表现。
...
### [74] [Benchmarking ORCA PT-1 Boson Sampler in Simulation](https://arxiv.org/abs/2505.23217)
*Jessica Park,Susan Stepney,Irene D'Amico*

Main category: quant-ph

TL;DR: ORCA Computing的TBI模拟器在解决中等规模（n<250）支配集问题时表现与经典算法相当，但模拟器的时间复杂度较差，实际设备可能更优。


<details>
  <summary>Details</summary>
Motivation: 评估ORCA的时间分束干涉仪（TBI）在解决计算问题（如支配集问题）中的性能，并与经典算法对比。

Method: 使用ORCA TBI模拟器解决支配集问题，不考虑噪声和误差，模拟其理想性能。

Result: TBI模拟器在中等规模问题上表现与线性规划和贪心算法相当，但时间扩展性较差。

Conclusion: TBI在实际设备中可能优于模拟器表现，适用于特定规模的问题。
...
### [75] [Graded Paraparticle Algebra of Majorana Fields for Multidimensional Quantum Computing with Structured Light](https://arxiv.org/abs/2505.23232)
*Fabrizio Tamburini,Nicolò Leone,Matteo Sanna,Roberto Siagri*
...
### [76] [A New Scaling Function for QAOA Tensor Network Simulations](https://arxiv.org/abs/2505.23256)
*Goro Miki,Yasuhiro Tokura*

Main category: quant-ph

TL;DR: 该研究利用张量网络在经典计算机上模拟量子近似优化算法（QAOA），探索纠缠与量子优势的关系，并扩展了先前研究的标度关系。


<details>
  <summary>Details</summary>
Motivation: 随着量子计算机的快速发展，评估量子算法性能的需求增加。张量网络因其在经典计算机上模拟量子算法的能力以及与纠缠的关联而备受关注。

Method: 通过张量网络模拟QAOA，分析其近似率与性能的关系，并扩展标度关系的研究。

Result: 发现标度关系在纠缠熵作为纵轴时仍成立，并提出了新的标度关系函数。此外，还揭示了QAOA中纠缠行为的规律。

Conclusion: 该研究为先前标度关系的理论基础提供了新见解，并有望推动量子算法性能评估的发展。
...
### [77] [Intermediate State Formation of Topologically Associated Chromatin Domains using Quantum Annealing](https://arxiv.org/abs/2505.23289)
*Tobias Kempe,S. M. Ali Tabei,Mohammad H. Ansari*

Main category: quant-ph

TL;DR: 论文提出了一种量子退火方法，用于高效采样染色质状态，解决了传统方法在高挫折和密集耦合情况下的困难。


<details>
  <summary>Details</summary>
Motivation: 研究染色质拓扑关联域的3D折叠机制与1D表观遗传景观之间的联系，目前尚不明确。

Method: 将染色质建模为自旋系统，利用量子退火方法嵌入表观遗传Ising模型到量子处理器拓扑中。

Result: 量子退火方法能够高效采样染色质状态。

Conclusion: 量子退火为研究染色质3D折叠机制提供了新工具。
...
### [78] [A hierarchy of thermodynamically consistent quantum operations](https://arxiv.org/abs/2505.23360)
*Fereshte Shahbeigi,M. Hamed Mohammady*

Main category: quant-ph

TL;DR: 论文研究了量子操作和测量在热力学定律约束下的可行性，分类了符合不同热力学定律的操作和测量，并分析了其不可实现性的原因。


<details>
  <summary>Details</summary>
Motivation: 确定量子操作和测量在热力学框架下的可行性，明确不同热力学定律对其的限制。

Method: 通过层次化分类，分析符合弱第三定律、强第三定律及第二和第三定律的量子操作和测量。

Result: 通道操作符合特定热力学定律的条件被明确，而测量虽不受热力学限制，但状态更新规则受约束。

Conclusion: 热力学定律对量子操作和测量的限制被系统化，为理解其可实现性提供了理论依据。
...
### [79] [Optimal Control by Variational Quantum Algorithms](https://arxiv.org/abs/2505.23373)
*Tangyou Huang,Jing-Jun Zhu,Zhong-Yi Ni*

Main category: quant-ph

TL;DR: 该论文提出了一种混合量子-经典算法框架，用于解决量子控制问题，并通过变分量子算法在一维自旋模型中实现了接近量子速度极限的完美状态转移。


<details>
  <summary>Details</summary>
Motivation: 量子控制问题在近期量子计算机上的解决具有重要潜力，但需要结合经典优化与量子模拟的方法。

Method: 采用混合框架，结合数字量子模拟和经典优化，引入控制最优性度量，并通过变分量子算法实现时间最优控制。

Result: 在一维自旋模型中实现了接近量子速度极限的完美状态转移，并展示了梯度行为和误差鲁棒性。

Conclusion: 该研究为近期量子平台上的量子控制问题提供了系统性的混合算法框架。
...
### [80] [Dynamic Estimation Loss Control in Variational Quantum Sensing via Online Conformal Inference](https://arxiv.org/abs/2505.23389)
*Ivana Nikoloska,Hamdi Joudeh,Ruud van Sloun,Osvaldo Simeone*

Main category: quant-ph

TL;DR: 提出了一种动态变分量子传感（VQS）框架，结合在线共形推理技术，为NISQ设备提供可靠的量子传感解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前变分量子传感方法缺乏严格的性能保证，且NISQ设备面临噪声和采样限制。

Method: 提出动态更新变分参数的在线控制框架，利用在线共形推理技术生成具有确定性误差范围的估计。

Result: 在量子磁力测量任务中验证了方法的可靠性，同时保持了估计的精确性。

Conclusion: 结合变分量子算法与在线共形推理，为NISQ设备实现可靠量子传感提供了实用方案。
...
### [81] [Enhancing the sensitivity of quantum optomechanical gyroscope by optical Kerr effect](https://arxiv.org/abs/2505.23453)
*Ying Liu,Rui Zhang,Wen-Quan Yang,Ya-Feng Jiao,Wang-Jun Lu,Qing-Shou Tan,Le-Man Kuang*

Main category: quant-ph

TL;DR: 提出一种通过光学Kerr效应增强量子光机械陀螺仪（QOMG）灵敏度的理论方案，利用量子Fisher信息（QFI）评估其计量潜力，发现Kerr相互作用显著提升灵敏度，并观察到超Heisenberg标度的参数估计精度。
...
Method: 利用量子Fisher信息评估QOMG的计量潜力，研究Kerr相互作用对灵敏度的影响，并分析驱动和耗散对QFI的作用。

Result: Kerr相互作用显著提升QOMG灵敏度，观察到超Heisenberg标度的参数估计精度，且灵敏度可通过驱动调控，但耗散会降低灵敏度。
...
### [82] [Spin polarized enantio-sensitive multipolar photoelectron currents](https://arxiv.org/abs/2505.23460)
*Philip Caesar M. Flores,Stefanos Carlstr Carlstrom,Serguei Patchkovskii,Andres F. Ordonez,Olga Smirnova*

Main category: quant-ph

TL;DR: 论文展示了通过光电子自旋检测在一光子电离中实现多极电流的新方法，揭示了分子手性的新特征。


<details>
  <summary>Details</summary>
Motivation: 研究目的是探索手性分子在光电子发射中的多极电流特性，尤其是在一光子电离条件下如何通过自旋检测揭示分子手性。

Method: 采用圆偏振光对随机取向的手性分子进行一光子电离，并通过光电子自旋检测分析多极电流的三种组分。

Result: 发现三种与分子手性相关的自旋极化电流组分，其强度可与光电子圆二色性（PECD）相比或更大。

Conclusion: 光电子自旋检测为一光子电离中的多极电流提供了新的研究视角，为分子手性分析提供了新工具。
...
### [83] [Extending and measuring dephasing times of nuclear spins in NV centers of diamond](https://arxiv.org/abs/2505.23512)
*Jingfu Zhang,Swathi S. Hegde,Fedor Jelezko,Dieter Suter*

Main category: quant-ph

TL;DR: 提出了测量和延长核自旋退相干时间的方法，使其超过电子自旋的纵向弛豫时间。


<details>
  <summary>Details</summary>
Motivation: 核自旋具有长退相干时间，但受电子自旋寿命限制，需解决这一问题以提升量子技术性能。

Method: 提出并实施了测量和延长核自旋退相干时间的方案。

Result: 实现了退相干时间超过电子自旋纵向弛豫时间T1。
...
### [84] [Scalable decoding protocols for fast transversal logic in the surface code](https://arxiv.org/abs/2505.23567)
*Mark L. Turner,Earl T. Campbell,Ophelia Crawford,Neil I. Gillespie,Joan Camps*

Main category: quant-ph

TL;DR: AMO量子计算因高连接性和长相干时间具有潜力，但提取错误症状速度较慢。横向逻辑可加速逻辑时钟，但现有解码器不可扩展。本文提出两种新的窗口解码协议，显著提升横向逻辑速度，为AMO量子比特的大规模算法提供可行性证据。


<details>
  <summary>Details</summary>
Motivation: 解决AMO量子计算中错误症状提取速度慢的问题，同时利用横向逻辑的高连接性优势，提升逻辑时钟速度。

Method: 提出两种新的窗口解码协议，恢复解码问题的模块化和局部性，以支持横向逻辑在表面码中的应用。

Result: 通过新解码协议，以极小的空间开销实现横向逻辑速度的十倍提升，优于传统晶格手术方法。

Conclusion: 新解码协议为AMO量子比特的大规模算法提供了可行性支持，展示了横向逻辑的潜力。
...
### [85] [Quantum Hilbert Transform](https://arxiv.org/abs/2505.23581)
*Nitin Jha,Abhishek Parakh*

Main category: quant-ph

TL;DR: 论文提出了量子希尔伯特变换（QHT）的公式，并将其应用于量子隐写协议，填补了量子信号处理领域的空白。


<details>
  <summary>Details</summary>
Motivation: 希尔伯特变换在信号处理中广泛应用，但缺乏量子版本，限制了量子信号处理的发展。

Method: 通过结合经典相移技术和量子操作，提出了QHT的公式。

Result: QHT为量子信号处理、通信、传感和安全信息隐藏开辟了新途径。

Conclusion: QHT的引入填补了量子信号处理领域的空白，具有广泛的应用潜力。
...
### [86] [Towards A Global Quantum Internet: A Review of Challenges Facing Aerial Quantum Networks](https://arxiv.org/abs/2505.23603)
*Nitin Jha,Abhishek Parakh*

Main category: quant-ph

TL;DR: 论文探讨了利用无人机、气球或卫星构建空中量子网络的可能性，分析了其挑战与解决方案，并展望了量子互联网的未来。


<details>
  <summary>Details</summary>
Motivation: 地面量子网络的扩展性有限，通过空中链路（如无人机、气球或卫星）可以提升量子网络的覆盖范围和实用性。

Method: 文章综述了空中量子链路的原理、构建难点及可能的解决方案，提出将地面站、空中中继和卫星整合为无缝系统。

Result: 通过整合多种技术，空中量子网络有望实现更广泛的覆盖和更高的实用性。

Conclusion: 空中量子网络是迈向实用量子互联网的重要一步，但仍需克服技术挑战。
...
### [87] [Fast-tracking and disentangling of qubit noise fluctuations using minimal-data averaging and hierarchical discrete fluctuation auto-segmentation](https://arxiv.org/abs/2505.23622)
*Abhishek Agarwal,Lachlan P. Lindoy,Deep Lall,Sebastian E. de Graaf,Tobias Lindström,Ivan Rungger*

Main category: quant-ph

TL;DR: 提出了一种高时间分辨率的量子比特噪声表征方法，结合自动分段工具，能够分离和追踪多种噪声源的波动。


<details>
  <summary>Details</summary>
Motivation: 量子计算机性能受限于量子比特噪声及其时间波动，现有方法难以同时表征多源噪声。

Method: 开发了一种噪声表征框架，包括最小化测量的高分辨率方法和自动分段工具，无需人工干预。

Result: 在transmon量子比特上实现了毫秒级时间分辨率，识别了电荷奇偶性和双能级系统切换的噪声来源。

Conclusion: 该方法不仅揭示了噪声来源，还为量子比特校准和纠错提供了改进信息。
...
### [88] [Fischer Information of a Nonequilibrium Anharmonic Donor-Acceptor Rectifier](https://arxiv.org/abs/2505.23636)
*Bitap Raj Thakuria,Trishna Kalita,Javed Akhtar,Himangshu Prabal Goswami*

Main category: quant-ph

TL;DR: 研究了非平衡供体-受体量子整流器系统与非线性振动模式的耦合，通过量子主方程计算时间依赖的Fisher信息，发现供体和受体能量有最佳估计时间，而非线性模式在稳态下更易估计。
...
Method: 将振动动力学视为两能级和多能级系统，推导量子主方程计算时间依赖的Fisher信息，估计关键整流器参数。

Result: 受体能量在强耦合和高偏置下最易估计，供体能量敏感性有限，振动频率估计在低温下更优。

Conclusion: 为量子设备参数估计提供了理论框架，对量子传感和计量应用具有指导意义。
...
### [89] [Visualizing Three-Qubit Entanglement](https://arxiv.org/abs/2505.23638)
*Alfred Benedito,Germán Sierra*

Main category: quant-ph

TL;DR: 提出了一种图形化框架表示三量子比特态的纠缠，分析了纠缠类和类型的几何结构，并探讨了其与纠缠度量（tangle）的关系。


<details>
  <summary>Details</summary>
Motivation: 研究三量子比特态的几何表示及其与纠缠度量的联系，为理解纠缠的几何特性提供新视角。

Method: 通过几何分析纠缠类和类型，推导纠缠度量的界限，并提出几何表达式。

Result: 发现纠缠类与几何结构相关，并提出了非一般态下tangle和Cayley超行列式的几何表达式。

Conclusion: 几何框架为纠缠分析提供了新工具，并揭示了对称性破坏和能级排斥下真正三体纠缠的鲁棒性条件。
...
### [90] [Fluxonium as a control qubit for bosonic quantum information](https://arxiv.org/abs/2505.23641)
*Ke Nie,J. Nofear Bradford,Supriya Mandal,Aayam Bista,Wolfgang Pfaff,Angela Kou*

Main category: quant-ph

TL;DR: 本文研究了使用fluxonium作为控制量子比特来避免超导谐振腔中的非线性效应，展示了其在高性能量子纠错中的潜力。


<details>
  <summary>Details</summary>
Motivation: 超导谐振腔在量子纠错中具有硬件效率优势，但传统控制量子比特（如transmon）会引入有害的非线性效应和退相干。fluxonium的长寿命和可控性为解决这一问题提供了可能。

Method: 通过实验验证fluxonium作为控制量子比特的性能，包括测量存储谐振腔的相干性、预测非线性效应，以及实现通用的量子态制备和表征。

Result: 实验证明fluxonium能够有效控制谐振腔，并预测其可以消除有害的非线性效应，优于transmon。

Conclusion: fluxonium作为高性能控制量子比特，有望在超导谐振腔的量子纠错中发挥重要作用。
...
### [91] [Markovian dissipation can stabilize a (localization) quantum phase transition](https://arxiv.org/abs/2505.22721)
...
### [92] [Measuring topological invariants of even-dimensional non-Hermitian systems through quench dynamics](https://arxiv.org/abs/2505.23633)
...
### [93] [Markovian dissipation can stabilize a (localization) quantum phase transition](https://arxiv.org/abs/2505.22721)
*Naushad A. Kamar,Mostafa Ali,Mohammad Maghrebi*

Main category: cond-mat.quant-gas

TL;DR: 论文展示了一种由马尔可夫耗散稳定的非平庸量子相变，区别于传统平衡态相变。


<details>
  <summary>Details</summary>
Motivation: 探索远离平衡态的量子相变，特别是在驱动开放量子系统中，耗散如何稳定而非破坏量子行为。

Method: 研究了一种改进的自旋-玻色子模型，其中自旋被驱动，玻色子受频率相关的马尔可夫损耗影响。

Result: 稳态表现出局域化相变，自旋动力学冻结，且相变发生时稳态变为纯态。
...
### [94] [Measuring topological invariants of even-dimensional non-Hermitian systems through quench dynamics](https://arxiv.org/abs/2505.23633)
*Xiao-Dong Lin,Long Zhang*

Main category: cond-mat.quant-gas

TL;DR: 提出了一种通过淬火动力学直接测量偶数维非厄米拓扑不变量的通用框架。


<details>
  <summary>Details</summary>
Motivation: 非厄米拓扑不变量的精确测定对研究非厄米拓扑相至关重要。

Method: 通过构造与原始非厄米哈密顿量拓扑等价的辅助厄米矩阵，利用带反转表面（BISs）进行拓扑表征。

Result: 展示了非厄米拓扑不变量可以通过BISs上的淬火后自旋纹理的缠绕模式提取。

Conclusion: 为量子平台中检测非厄米拓扑不变量提供了实验可行的方案。
...
### [95] [Eigenstate Thermalization Hypothesis (ETH) for off-diagonal matrix elements in integrable spin chains](https://arxiv.org/abs/2505.23602)
...
### [96] [Eigenstate Thermalization Hypothesis (ETH) for off-diagonal matrix elements in integrable spin chains](https://arxiv.org/abs/2505.23602)
*Federico Rottoli,Vincenzo Alba*

Main category: cond-mat.stat-mech

TL;DR: 研究了可积自旋链中局部算符的非对角矩阵元，重点分析了各向同性自旋-1/2海森堡链（XXX链）。利用代数Bethe Ansatz方法高效计算了两态间的矩阵元，并分析了同一和不同热力学宏观态下的行为。


<details>
  <summary>Details</summary>
Motivation: 探讨可积系统中局部算符矩阵元的性质，揭示其在热力学宏观态下的统计行为。

Method: 采用代数Bethe Ansatz技术，计算支持两点的算符在能量本征态间的矩阵元，并数值分析其衰减规律。

Result: 同一宏观态下矩阵元呈指数衰减，符合Gumbel分布；不同宏观态下衰减更快，仍符合Gumbel分布。

Conclusion: 矩阵元的衰减行为与宏观态相关，且统计特性可由Gumbel分布描述。
...
### [97] [The Multiverse: a Philosophical Introduction](https://arxiv.org/abs/2505.23639)
*Jeremy Butterfield*
...
### [98] [The Multiverse: a Philosophical Introduction](https://arxiv.org/abs/2505.23639)
*Jeremy Butterfield*

Main category: physics.hist-ph

TL;DR: 本书探讨了多元宇宙的概念，分析了哲学和物理学中的三种版本：逻辑可能世界、量子理论的多世界解释和膨胀宇宙学中的气泡宇宙。


<details>
  <summary>Details</summary>
Motivation: 研究多元宇宙的哲学和物理学意义，探讨其背后的核心问题。

Method: 通过哲学和物理学视角，分别讨论三种多元宇宙理论，并针对每种理论提出一个主要哲学问题。

Result: 分析了多元宇宙的不同理论及其哲学问题，并对这些理论进行了比较。

Conclusion: 多元宇宙的概念在哲学和物理学中具有重要意义，但不同理论之间存在差异和争议。
...
### [99] [Quantum-Based Software Engineering](https://arxiv.org/abs/2505.23674)
*Jianjun Zhao*
...
### [100] [Quantum-Based Software Engineering](https://arxiv.org/abs/2505.23674)
*Jianjun Zhao*

Main category: cs.SE

TL;DR: 论文提出量子计算在软件工程中的应用方向（QBSE），探讨其范围、与量子软件工程（QSE）的区别，并总结现有研究，提出初步研究议程。


<details>
  <summary>Details</summary>
Motivation: 量子计算在解决复杂计算问题上具有潜力，软件工程中的许多任务（如测试用例选择、缺陷预测等）可能受益于量子优化、搜索和学习技术。

Method: 提出Quantum-Based Software Engineering (QBSE)概念，明确其范围与QSE的区别，总结现有研究，并提出初步研究议程。

Result: 识别了可能受益于量子技术的软件工程问题类型，并整理了现有研究。

Conclusion: QBSE是一个有潜力的研究方向，未来需要结构化发展以推动其在软件工程中的应用。
...
### [101] [Space magnetometry with a differential atom interferometer](https://arxiv.org/abs/2505.23532)
...
### [102] [Space magnetometry with a differential atom interferometer](https://arxiv.org/abs/2505.23532)
*Matthias Meister,Gabriel Müller,Patrick Boegel,Albert Roura,Annie Pichery,David B. Reinhardt,Timothé Estrampes,Jannik Ströhle,Enno Giese,Holger Ahlers,Waldemar Herr,Christian Schubert,Éric Charron,Holger Müller,Jason R. Williams,Ernst M. Rasel,Wolfgang P. Schleich,Naceur Gaaloul,Nicholas P. Bigelow*

Main category: physics.atom-ph

TL;DR: 论文研究了在太空环境中使用差分原子干涉仪进行轨道磁力测量的方法，展示了高精度量子传感任务的潜力。


<details>
  <summary>Details</summary>
Motivation: 太空中的原子干涉仪可用于高精度测量、导航和地球观测，差分干涉仪能抑制共模噪声，适合在环境噪声下进行可靠测量。

Method: 在NASA的冷原子实验室中，使用差分单环和双环干涉仪进行轨道磁力测量，通过比较磁敏感和不敏感状态的原子测量结果实现磁场曲率映射。

Result: 实验成功实现了原子磁力计对磁场曲率的测量。

Conclusion: 研究结果为未来太空中的高精度量子传感任务奠定了基础。
...
### [103] [Telecom quantum dots on GaAs substrates as integration-ready high performance single-photon sources](https://arxiv.org/abs/2505.22886)
...
### [104] [Fermion parity and quantum capacitance oscillation with partially separated Majorana and quasi-Majorana modes](https://arxiv.org/abs/2505.23741)
...
### [105] [Telecom quantum dots on GaAs substrates as integration-ready high performance single-photon sources](https://arxiv.org/abs/2505.22886)
*Beatrice Costa,Bianca Scaparra,Xiao Wei,Hubert Riedl,Gregor Koblmüller,Eugenio Zallo,Jonathan Finley,Lukas Hanschke,Kai Müller*

Main category: cond-mat.mes-hall

TL;DR: 论文研究了在电信O波段和C波段发射的半导体量子点的光学特性和单光子发射，展示了其在量子通信和光子量子计算中的潜力。


<details>
  <summary>Details</summary>
Motivation: 开发在电信波段发射的确定性单光子源对量子通信和光子量子计算至关重要。
...
### [106] [Fermion parity and quantum capacitance oscillation with partially separated Majorana and quasi-Majorana modes](https://arxiv.org/abs/2505.23741)
*Tudor D. Stanescu,Sumanta Tewari*

Main category: cond-mat.mes-hall

TL;DR: 论文研究了量子电容的磁通量依赖性振荡现象，指出这种现象不仅源于拓扑保护的Majorana零模，还可能由部分分离的Majorana模或拓扑平凡相中的准Majorana模引起。


<details>
  <summary>Details</summary>
Motivation: 探索量子电容振荡现象的起源，澄清其与拓扑Majorana零模的关系。

Method: 通过实验和理论分析，研究了半导体超导体量子点系统中磁通量诱导的量子电容振荡。

Result: 发现振荡现象可能由多种Majorana模引起，不能单独作为拓扑Majorana零模的证据。

Conclusion: 量子电容振荡现象的观测是重要进展，但需结合其他证据才能确认拓扑Majorana零模的存在。
...
### [107] [Ultra-long-living magnons in the quantum limit](https://arxiv.org/abs/2505.22773)
...
### [108] [Ultra-long-living magnons in the quantum limit](https://arxiv.org/abs/2505.22773)
*Rostyslav O. Serha,Kaitlin H. McAllister,Fabian Majcen,Sebastian Knauer,Timmy Reimann,Carsten Dubs,Gennadii A. Melkov,Alexander A. Serga,Vasyl S. Tyberkevych,Andrii V. Chumak,Dmytro A. Bozhko*

Main category: cond-mat.mtrl-sci

TL;DR: 观察到短波长磁振子在极低温下寿命超过18微秒，为量子计算提供了新的数据载体可能性。


<details>
  <summary>Details</summary>
Motivation: 磁振子寿命短限制了其作为量子信息载体的应用，研究旨在突破这一限制。

Method: 在超纯单晶钇铁石榴石球中，从常温到30 mK的宽温度范围内进行实验。

Result: 观察到短波长磁振子寿命超过18微秒。

Conclusion: 研究结果为现代固态量子计算平台中使用磁振子作为数据载体开辟了新途径。
...
### [109] [Signatures of Correlation of Spacetime Fluctuations in Laser Interferometers](https://arxiv.org/abs/2505.22892)
...
### [110] [Signatures of Correlation of Spacetime Fluctuations in Laser Interferometers](https://arxiv.org/abs/2505.22892)
*B. Sharmila,Sander M. Vermeulen,Animesh Datta*

Main category: gr-qc

TL;DR: 论文探讨了利用激光干涉仪检测时空波动（SFs）的方法，并分析了不同重力模型与干涉仪几何结构的对应关系，为未来实验设计和信号识别提供指导。


<details>
  <summary>Details</summary>
Motivation: 研究旨在建立时空波动与不同重力模型之间的对应关系，以指导未来干涉仪的设计和实验数据的信号识别。

Method: 分析了三种不同类别的时空波动，基于其两点相关函数的衰减行为和对称性，并研究了Michelson激光干涉仪（如LIGO、QUEST和GQuEST）的输出信号。

Result: 实验室规模的干涉仪（如QUEST和GQuEST）能够观测到所有类别的时空波动特征信号，而LIGO更适合检测时空波动的存在与否。

Conclusion: 不同规模的干涉仪各有优势，实验室设备能提供更多关于时空波动性质的信息，而LIGO更适合初步检测。
